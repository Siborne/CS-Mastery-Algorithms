...
public class Code02_SuperPalindromes {
	...
	public int superpalindromesInRange1(String left, String right) {
	...
	private static long evenEnlarge(long seed) {
		long ans = seed;
		while (seed != 0) {
			ans = ans * 10 + seed % 10;
			seed /= 10;
	...
	private static long oddEnlarge(long seed) {
		long ans = seed;
		seed /= 10;
		while (seed != 0) {
			ans = ans * 10 + seed % 10;
	...
	private static boolean check(long ans, long l, long r) {
	...
	private static long[] records = new long[] {
			1L,
			4L,
			9L,
			121L,
	...
	private static List<Long> collect() {
		long l = 1;
